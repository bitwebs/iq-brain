...
const ValidatorActions = ({ destination }: { destination: ValAddress }) => {
  ...
  const renderDelegationsValue = () => {
    ...
    const value = calcValue({ amount, denom: "ubiq" })

    return (
      <section>
        <Read amount={amount} denom="ubiq" className={styles.total} block />
        {currency !== "ubiq" && (
          <Read
            amount={value}
            denom={currency}
  ...
  const renderRewardsValue = () => {
    ...
    const amount = list.find(({ denom }) => denom === "ubiq")?.amount ?? "0"

    return (
      <section>
        <Read amount={amount} denom="ubiq" className={styles.total} />{" "}
